    ...
    def get(self):
        ...
        if sqlGrade==None:
            ...
            for i in range(gradeleve,localYear):
                ...
                while True:
                    if ("Content" in models.globaldata.backmessage) and models.globaldata.backmessage['Content'].find(str(i)+'-'+str(i+1)+'-1')!=-1:
                ...
                while True:
                    if ("Content" in models.globaldata.backmessage) and models.globaldata.backmessage['Content'].find(str(i)+'-'+str(i+1)+'-2')!=-1: 
            ...
            if int(localtime[5:7])<6 :
                ...
                while True:
                    if ("Content" in models.globaldata.backmessage) and models.globaldata.backmessage['Content'].find(localtime[0:4]+"-"+str(int(localtime[0:4])+1)+"-1")!=-1:
                        ...
                        break
            elif int(localtime[5:7])>=6 :
                itchat.send_msg("CJCX1 "+str(int(localtime[0:4])-1)+"-"+localtime[0:4]+"-2", toUserName=models.globaldata.mps)
                while True:
                    if ("Content" in models.globaldata.backmessage) and models.globaldata.backmessage['Content'].find(str(int(localtime[0:4])-1)+"-"+localtime[0:4]+"-2")!=-1: 
                        temp_Json=self.HandleWx2Json(models.globaldata.backmessage['Content'])
                        if temp_Json:
                            Result_Json[str(int(localtime[0:4])-1)+"-"+localtime[0:4]+"-2"]=temp_Json
                        break
            if Result_Json:
                if  (str(int(localtime[0:4])-1)+"-"+localtime[0:4]+"-2" in sqlGrade) or (localtime[0:4]+"-"+str(int(localtime[0:4])+1)+"-1" in sqlGrade):
                    if sqlGrade[list(Result_Json.keys())[0]].keys() ^ Result_Json.keys():
